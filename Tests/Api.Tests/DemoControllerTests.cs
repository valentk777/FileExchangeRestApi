...
//		httpClient.Setup(x => x.GetFileAsync(fileName)).ReturnsAsync(() => _responseFile);
//		var controller = new DemoController(_logger, httpClient.Object);

//		var response = await controller.DownloadFile(fileName);
...
//		var response = await controller.GetFileAsync(incorrectFileName);
...
//		var httpClient = new Mock<IDemoHttpClient>();
//		httpClient.Setup(x => x.GetFileAsync(fileName)).ReturnsAsync(() => new DemoDomainFile(fileName, fileContent));
//		var controller = new DemoController(_logger, httpClient.Object);

//		var response = await controller.GetFileAsync(fileName);
...
//		httpClient.Setup(x => x.GetFileAsync(fileName)).ThrowsAsync(new HttpRequestException(exceptionMessage));
//		var controller = new DemoController(_logger, httpClient.Object);
...
//		var httpClient = new Mock<IDemoHttpClient>();
//		var exceptionMessage = "exception message";
//		httpClient.Setup(x => x.GetFileAsync(fileName)).ThrowsAsync(new Exception(exceptionMessage));
//		var controller = new DemoController(_logger, httpClient.Object);
